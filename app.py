...
def crawl_novel()
    ...
    if kakuyomu.jpworks not in url
    ...
    try
        ...
        content = nn.join(p.get_text(strip=True) for p in paragraphs)
        ...
        return jsonify({error fFailed to fetch the URL {e}}), 500
    except Exception as e
        return jsonify({error fAn unexpected error occurred {e}}), 500
